...
## Corrección telemetría HTTP 422

La ruta estática `POST /api/telemetry/simulate` se declara antes de `POST /api/telemetry/{equipment_id}` para evitar que FastAPI interprete `simulate` como un identificador de equipo. Esta corrección evita el error `422 Unprocessable Entity` que aparecía en el streaming automático.
